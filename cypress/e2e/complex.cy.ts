describe("Complex File Operations Tests", () => {
  ...
  it("[CPX-01][bug] should create file in current directory after selecting existing file", () => {
    ...
    // 验证编辑器标题
    cy.contains("h2", "Editing: folder/subfolder/existing.txt").should("be.visible");

    // 创建新文件
    cy.get("input[placeholder*='Enter name']")
      .type("new-file.txt{enter}");
    ...
    cy.wait("@putFile").then((interception) => {
      expect(interception.request.url)
        .to.include("folder/subfolder/new-file.txt");
      expect(interception.response?.statusCode).to.equal(200);
    });

    // 验证编辑器标题更新
    cy.contains("h2", "Editing: folder/subfolder/new-file.txt")
      .should("be.visible");
  });
});
